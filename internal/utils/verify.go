...
func VerifyFinding(cfg *config.ScanConfig, finding config.Finding) (bool, error) {
	if IsScanCancelled() {
		return false, fmt.Errorf("scan cancelled")
	}

	config.DebugPrintf(cfg, "[*] Verifying finding: %s in parameter %s", finding.Type, finding.Parameter)
...
func verifyXSS(cfg *config.ScanConfig, finding config.Finding) (bool, error) {
	baseURL, params, err := httpclient.ExtractParamsFromURL(finding.URL)
	if err != nil {
		return false, err
	}
	...
	respBody, _, err := httpclient.FetchURL(cfg, baseURL+"?"+params.Encode(), "GET", nil, nil)
	if err != nil {
		return false, err
	}
...
func verifySQLi(cfg *config.ScanConfig, finding config.Finding) (bool, error) {
	baseURL, params, err := httpclient.ExtractParamsFromURL(finding.URL)
	if err != nil {
		return false, err
	}
...
func verifyLFI(cfg *config.ScanConfig, finding config.Finding) (bool, error) {
	baseURL, params, err := httpclient.ExtractParamsFromURL(finding.URL)
	if err != nil {
		return false, err
	}
...
func verifyRCE(cfg *config.ScanConfig, finding config.Finding) (bool, error) {
	baseURL, params, err := httpclient.ExtractParamsFromURL(finding.URL)
	if err != nil {
		return false, err
	}
...
func verifySSRF(cfg *config.ScanConfig, finding config.Finding) (bool, error) {
	baseURL, params, err := httpclient.ExtractParamsFromURL(finding.URL)
	if err != nil {
		return false, err
	}
...
func verifySSTI(cfg *config.ScanConfig, finding config.Finding) (bool, error) {
	baseURL, params, err := httpclient.ExtractParamsFromURL(finding.URL)
	if err != nil {
		return false, err
	}
...
func verifyNoSQLi(cfg *config.ScanConfig, finding config.Finding) (bool, error) {
	baseURL, params, err := httpclient.ExtractParamsFromURL(finding.URL)
	if err != nil {
		return false, err
	}
...
func verifyXXE(cfg *config.ScanConfig, finding config.Finding) (bool, error) {
	baseURL, params, err := httpclient.ExtractParamsFromURL(finding.URL)
	if err != nil {
		return false, err
	}
...
func verifyOpenRedirect(cfg *config.ScanConfig, finding config.Finding) (bool, error) {
	baseURL, params, err := httpclient.ExtractParamsFromURL(finding.URL)
	if err != nil {
		return false, err
	}
	...
	params.Set(finding.Parameter, testPayload)
	_, headers, err := httpclient.FetchURL(cfg, baseURL+"?"+params.Encode(), "GET", nil, nil)
	if err != nil {
		return false, err
	}
...
func VerifyAllFindings(cfg *config.ScanConfig) {
