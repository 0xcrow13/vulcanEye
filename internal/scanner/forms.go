...
func FindFileUploadForms(htmlStr string) []config.UploadForm {
	var uploadForms []config.UploadForm
	tokenizer := html.NewTokenizer(strings.NewReader(htmlStr))
	inForm := false
	var cur config.UploadForm
	var enctype string
...
func ScanFileUploadForms(cfg *config.ScanConfig, pageURL string, forms []config.UploadForm) {
	if len(forms) == 0 {
		output.PrintBullet(config.ColorGreen, "No file upload forms found on this page")
		return
	}
	for _, form := range forms {
		...
		output.PrintBullet(config.ColorCyan, fmt.Sprintf("Scanning file field '%s' via form action '%s'", form.FileField, form.Action))
		fileName := "pwntest.php"
		fileContent := []byte("<?php echo 'pwntwouploadmarker'; ?>")
		otherFields := form.OtherFields
		respBody, _, err := httpclient.FetchMultipart(cfg, fullURL, otherFields, form.FileField, fileName, fileContent, nil)
		if err != nil {
		...
		for _, m := range successMarkers {
			if strings.Contains(respBody, m) {
				...
				fmt.Printf("    %sFile field    :%s %s%s\n", config.ColorYellow, config.ColorReset, form.FileField, config.ColorReset)
				fmt.Printf("    %sUploaded file :%s %s%s\n", config.ColorYellow, config.ColorReset, fileName, config.ColorReset)
				fmt.Printf("    %sUpload URL    :%s %s%s\n", config.ColorYellow, config.ColorReset, fullURL, config.ColorReset)
				vuln = true
			}
		}
		if !vuln {
			output.PrintBullet(config.ColorGreen, fmt.Sprintf("No file upload vulnerabilities in field '%s'", form.FileField))
		}
	}
}

// Debug print for HTML before parsing forms (add this in your main scanning function)
func debugPrintHTML(cfg *config.ScanConfig, pageBody string) {
	if cfg.Debug {
